...
func createTree(max int) BTree {
	head := BTreeNode{parent: nil, keys: make([]int, 0), children: make([]*BTreeNode, 0)}
...
func (bt BTree) insertSearch(key int) (*BTreeNode, bool) {
	cursor := bt.head
...
func (bt *BTree) Insert(key int) {
	...
	if found {
		fmt.Println("ne valja kljuc vec postoji")
	}
...
func (bt *BTree) razdvoj(cursor *BTreeNode) {
	...
	if parent == nil {
		parent = &BTreeNode{parent: nil, children: []*BTreeNode{cursor}, keys: make([]int, 0)}
		bt.head = parent
		bt.height++
...
func findElem(cursor *BTreeNode, key int) int {
	for i := 0; i < len(cursor.keys); i++ {
...
func findNode(cursor *BTreeNode, key int) int {
	cursor = cursor.parent
	if cursor == nil {
		return 0
	}
...
func merge2(levi *BTreeNode, desni *BTreeNode) {
	if len(levi.children) != 0 {
		//levi = levi.children[len(levi.children)-1]
		//desni = desni.children[0]
		merge2(levi.children[len(levi.children)-1], desni.children[0])
	} else {
...
func merge(levi, desni, parentt *BTreeNode) {
	if len(levi.children) != 0 {
		merge(levi.children[len(levi.children)-1], desni.children[0], parentt)
	} else {
		levi.keys = append(levi.keys, desni.keys...)
		parent := desni.parent
		if parent == parentt {
